    ...
    <script>
        $(document).ready(function() {
            ...
            function filterDayNow(provinceId = null, regencieId = null, subdasId = null) {
                $.ajax({
                    url: "{{url('api/getAllQualityWater')}}",
                    method: 'GET',
                    data: {
                        provinceId: provinceId,
                    ...
                    success: function(data) {
                        ...
                        $.each(data.data, function(index, value) {
                            let bgStatus = '';
                            if (value.status === 'Kondisi Baik') {
                                bgStatus += 'rgb(80, 200, 120)';
                            } else if(value.status === 'Cemar Ringan') {
                                bgStatus += 'rgb(228, 208, 10)';
                            } else if(value.status === 'Cemar Sedang') {
                                bgStatus += 'rgb(255, 140, 0)';
                            } else if(value.status === 'Cemar Berat') {
                                bgStatus += 'rgb(255, 87, 51)';
                            } else {
                                bgStatus += 'rgb(80, 200, 115)';
                            }
            ...
            function getGeoJsonmarker(data) {
                ...
                let geojson = {
                    ...
                    "features": data.data.map(marker => {
                        ...
                        if (marker.totla === 'Kondisi Baik') {
                            statusColor += 'success';
                            iconUrl = "{{asset('img/marker-icon-2x-green.png')}}";
                        } else if(marker.status === 'Cemar Ringan') {
                            statusColor += 'warning';
                            iconUrl = "{{asset('img/marker-icon-2x-yellow.png')}}"; 
                        } else if(marker.status === 'Cemar Sedang') {
                            statusColor += 'danger';
                            iconUrl = "{{asset('img/marker-icon-2x-orange.png')}}"; 
                        } else if(marker.status === 'Cemar Berat') {
                            statusColor += 'danger';
                                                ...
                                                <div class="leaflet-popup-content" style="width: 350px;">
                                                    <div class="p-3 row">
                                                        <div class="alert alert-${statusColor}">
                                                            Kualitas Air Dalam Kondisi : ${marker.status}    
                                                        </div>
                                                        <div class="col">
                                                            <h5>${marker.nama}</h5>
                                                        ...
                                                        <div class="col">
                                                            ...
                                                            <div>
                                                                <div>
                                                                    <div class="mt-2 row">
                                                                        <div class="col-md-12 mt-3">Status : ${marker.total}</div>
                                                                        <div class="col-md-12 mt-3">Semester : ${marker.semester}</div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <hr class="divide">
                                                        <a class="btn" style="background-color: rgb(10, 71, 147); color: #fff" href="{{url('qualitys/${marker.id}/show')}}">Lihat Data Selengkapnya</a>
                                                    </div>
                                                </div>
                                            </div>
                                        <div>
                ...
                geoJsonLayer = L.geoJSON(geojson, {
                    pointToLayer: function(feature, latlng) {
                        let iconUrl = '';
                        if (feature.properties.status === 'Kondisi Baik') {
                            iconUrl = "{{asset('img/marker-icon-2x-green.png')}}";
                        } else if (feature.properties.status === 'Cemar Ringan') {
                            iconUrl = "{{asset('img/marker-icon-2x-yellow.png')}}"; 
                        } else if (feature.properties.status === 'Cemar Sedang') {
                            iconUrl = "{{asset('img/marker-icon-2x-orange.png')}}"; 
                        } else if (feature.properties.status === 'Cemar Berat') {
                            iconUrl = "{{asset('img/marker-icon-2x-red.png')}}"; 
                        } else {
                            iconUrl = "{{asset('img/marker-icon-2x-blue.png')}}"; 
                        }
